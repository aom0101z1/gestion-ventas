...
function renderPipelineView(contacts) {
    ...
    // ✅ ENHANCED FIX: Set up event listeners after DOM is fully rendered
    setTimeout(() => {
        try {
            setupPipelineEventListeners();
...
function setupDragAndDropListeners() {
    console.log('🔧 ENHANCED: Setting up drag and drop listeners...');
    ...
    return `
        ...
        ">
            <div style="display: flex; gap: 0.75rem; flex-wrap: wrap;">
                ...
                <button onclick="reloadPipelineClean()" class="btn btn-warning" style="
                    padding: 0.5rem 1rem;
                    background: #f59e0b;
                    color: white;
                    border: none;
            ...
            <div style="display: flex; align-items: center; gap: 1rem; font-size: 0.85rem; color: #6b7280;">
                ...
                <span style="padding: 0.25rem 0.5rem; background: #f0fdf4; color: #16a34a; border-radius: 4px;">
                    ✅ Drag & Drop Activo
                </span>
            </div>
        </div>
...
function reloadPipelineClean() {
    ...
    try {
        if (window.FirebaseData && typeof window.FirebaseData.getFilteredContacts === 'function') {
            window.FirebaseData.getFilteredContacts().then(contacts => {
                // Clean the data
                window.pipelineData = validateAndCleanPipelineData(contacts || []);
                pipelineData = window.pipelineData;
...
console.log('🔧 Console commands: reloadPipelineClean()');
