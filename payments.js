...
class HalfPageInvoiceGenerator {
    ...
    // Minimal implementation for now - add full methods later
    async printInvoices(invoices) {
        console.log('Printing half-page invoices:', invoices);
        // This will open a print dialog with the current page
        // Full implementation would generate the HTML layout
        window.print();
    }
    
    createInvoiceFromPayment(payment, student) {
        return {
            number: `CB-${new Date().getFullYear()}-TEST-001`,
            ...
            items: [{
                quantity: 1,
                description: `Pago ${payment.month}`,
...
window.printAsHalfPage = async function(invoiceNumber) {
    try {
        // Get invoice data
        const db = window.firebaseModules.database;
        const invoiceRef = db.ref(window.FirebaseData.database, `invoices/${invoiceNumber}`);
        ...
        if (!snapshot.exists()) {
            window.showNotification('❌ Comprobante no encontrado', 'error');
            return;
        }
        
        const invoiceData = snapshot.val();
        
        // Convert to half-page format
        const halfPageInvoice = {
            number: invoiceData.number,
            date: invoiceData.date,
            studentName: invoiceData.student.name,
            documentType: invoiceData.student.tipoDoc,
            documentNumber: invoiceData.student.nit,
            items: invoiceData.items,
            subtotal: invoiceData.subtotal,
            total: invoiceData.total,
            paymentMethod: invoiceData.paymentMethod,
            bank: invoiceData.bank,
            notes: invoiceData.observations,
            createdBy: invoiceData.createdBy || 'Sistema'
        };
        
        // Print single invoice (will appear on top half of page)
        await window.HalfPageInvoiceGen.printInvoices([halfPageInvoice]);
        
    } catch (error) {
        console.error('Error printing half-page:', error);
        window.showNotification('❌ Error al imprimir', 'error');
    }
};
